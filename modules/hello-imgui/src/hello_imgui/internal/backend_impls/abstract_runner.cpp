...
// For legacy reasons, there are still few references to OpenGL in this file, but this needs to be refactored
// out.
//
...
AbstractRunner::AbstractRunner(RunnerParams& params_) : params(params_) {}
...
{
    fprintf(stderr,
            "Detected a potential font loading error! You might try to reduce the number of loaded fonts "
            "and/or their size!\n");
#ifdef HELLOIMGUI_HAS_OPENGL
    if (ImGui::GetIO().FontGlobalScale < 1.f)
    {
        fprintf(stderr,
                "Trying to solve the font loading error by changing ImGui::GetIO().FontGlobalScale from %f "
                "to 1.f! Font rendering might be less crisp...\n",
...
{
    mGeometryHelper = std::make_unique<WindowGeometryHelper>(params.appWindowParams.windowGeometry,
...
bool AbstractRunner::ShallSizeWindowRelativeTo96Ppi()
{
    ...
    {
        bool doRestorePreviousGeometry =
            (params.appWindowParams.restorePreviousGeometry &&
             HelloImGuiIniSettings::LoadLastRunWindowBounds(IniSettingsLocation(params)).has_value());
...
float AbstractRunner::ImGuiDefaultFontGlobalScale()
{
    ...
    // This simplifies application development in most cases, but in our case we happen to render fonts at 1x
    // scale while screen renders at 2x scale.
...
void AbstractRunner::MakeWindowSizeRelativeTo96Ppi_IfRequired()
{
    if (ShallSizeWindowRelativeTo96Ppi())
    {
        ...
        if (scaleFactor != 1.f)
        {
            ...
            // update position
            if ((params.appWindowParams.windowGeometry.positionMode ==
                 ...
                 HelloImGui::WindowPositionMode::OsDefault))
            {
                ...
                ForDim2(dim) bounds.position[dim] = monitorBounds.Center()[dim] - bounds.size[dim] / 2;
            }
            else if (params.appWindowParams.windowGeometry.positionMode ==
                     HelloImGui::WindowPositionMode::FromCoords)
            {
                ForDim2(dim) bounds.position[dim] = (int)((float)bounds.position[dim] * scaleFactor);
            }
...
void AbstractRunner::SetLayoutResetIfNeeded()
{
    if (params.imGuiWindowParams.defaultImGuiWindowType == DefaultImGuiWindowType::ProvideFullScreenDockSpace)
    {
        ...
        {
            if (!HelloImGuiIniSettings::HasUserDockingSettingsInImguiSettings(IniSettingsLocation(params),
...
{
    // Notes:
    // - this method is rather long, but this is intentional in order to be able to see the logic flow at a
    // glance
    ...
    //           SCOPED_RELEASE_GIL_ON_MAIN_THREAD;       // SCOPED_RELEASE_GIL_ON_MAIN_THREAD does nothing if
    //           not using python
    //
    ...
    if (true)  // Register tests
    {
        ...
        if (mIdxFrame >= 1)
        {
            if (params.useImGuiTestEngine && params.callbacks.RegisterTests &&
                (!params.callbacks.registerTestsCalled))
            {
    ...
    {              // SCOPED_RELEASE_GIL_ON_MAIN_THREAD start
        ...
        // i/   On the first frame (mIdxFrame==0), we create a window, and use the user provided size (if
        // provided). The window is initially hidden.
        //      (this was done much sooner by mBackendWindowHelper)
        // ii/  On the second frame (mIdxFrame == 1), we may multiply this size by the Dpi factor (if > 1), to
        // handle windows and linux High DPI iii/ At the end of the second frame, we measure the size of the
        // widgets and use it as the application window size, if the user required auto size iv/  At the
        // beginning of the third frame (mIdxFrame==2 / mWasWindowAutoResizedOnPreviousFrame), we may apply
        // the auto-size and recenter the window to the center of the monitor v/   At the 4th frame (mIdxFrame
        // >= 3), we finally show the window Phew...

        // ii/ On the second frame (mIdxFrame == 1), we may multiply this size by the Dpi factor (if > 1), to
        // handle windows and linux High DPI
        if (mIdxFrame == 1)
        {
        ...
        // iv/ At the beginning of the third frame (mIdxFrame==2 / mWasWindowAutoResizedOnPreviousFrame), we
        // may apply the auto-size and recenter the window to the center of the monitor
        if (mWasWindowAutoResizedOnPreviousFrame)
        {
            ...
            // we do this on the third frame (mIdxFrame == 2), since the initial autosize happens on the
            // second (see WantAutoSize())
            if (params.appWindowParams.windowGeometry.positionMode ==
                    HelloImGui::WindowPositionMode::MonitorCenter &&
                (mIdxFrame == 2))
    ...
    {              // SCOPED_RELEASE_GIL_ON_MAIN_THREAD start
        ...
        // This test should be done after calling Impl_PollEvents() since it checks the event queue for
        // incoming events!
    ...
    // iii/ At the end of the second frame, we measure the size of the widgets and use it as the application
    // window size, if the user required auto size
...
// This test should be done after calling Impl_PollEvents() since it checks the event queue for incoming
// events!
